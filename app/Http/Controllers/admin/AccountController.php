    ...
    */
    public function dashboard() {
        ...
        try {
            ...
            // $data['seasons']            = Season::select('id','title','status')->where(['status' => '1'])->whereNull(['deleted_at'])->count();
            // $data['regions']            = Region::select('id','title','status')->where(['status' => '1'])->whereNull(['deleted_at'])->count();
            // $data['playerTypes']        = PlayerType::select('id','title','status')->where(['status' => '1'])->whereNull(['deleted_at'])->count();
            // $data['competitiveLevels']  = CompetitiveLevel::select('id','title','status')->where(['status' => '1'])->whereNull(['deleted_at'])->count();
            // $data['preferredHomeCourts']= PreferredHomeCourt::select('id','title','status')->where(['status' => '1'])->whereNull(['deleted_at'])->count();
            $data['players']            = User::where(['status' => '1','type' => 'U'])->whereNull(['role_id','deleted_at'])->count();
